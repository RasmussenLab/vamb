    ...
    def trainmodel(self,
        ...
        for epoch_i in range(nepochs):
            ...
            for depths_in, tnfs_in, _ in data_loader:  # weights currently unused here
                ...
                # Forward pass
                _,depths_out, tnfs_out, z_latent, y_latent ,d_z_latent, d_y_latent  = self(depths_in, tnfs_in, z_prior, y_prior)
                ...
                g_loss_adv_z = adversarial_loss(self._discriminator_z(z_latent), labels_prior) 
                g_loss_adv_y = adversarial_loss(self._discriminator_y(y_latent), labels_prior) 
               
                ed_loss = (1-self.sl) * vae_loss + (self.sl*self.slr) * g_loss_adv_z + (self.sl*(1-self.slr))* g_loss_adv_y 
                ...
                d_z_loss_prior = adversarial_loss(self._discriminator_z(z_prior), labels_prior)
                d_z_loss_latent = adversarial_loss(self._discriminator_z(z_latent), labels_latent)
                ...
                d_y_loss_prior = adversarial_loss(self._discriminator_y(y_prior), labels_prior)
                d_y_loss_latent = adversarial_loss(self._discriminator_y(y_latent), labels_latent)
...
#            # print trainning losses/others into log file 
            if logfile is not None:
                print('\tEpoch: {}\t Loss Enc/Dec: {:.6f}\t Rec. loss: {:.4f}\t CE: {:.4f}\tSSE: {:.4f}\t Dz loss: {:.7f}\t Dy loss: {:.6f}\t Batchsize: {}\t Epoch time(min): {: .4}'.format(
                    epoch_i + 1,
        ...
        with torch.no_grad():
            ...
            for depths_in, tnfs_in, _  in new_data_loader:
                ...
                if self.usecuda:
                    z_prior = torch.cuda.FloatTensor(I.shape[0], self.ld).normal_()
                    z_prior.cuda()
                    ohc = RelaxedOneHotCategorical(torch.tensor([0.15],device='cuda'),torch.ones([I.shape[0],self.y_len],device='cuda'))
                    y_prior = ohc.sample()
                    y_prior = y_prior.cuda()

                else:
                    z_prior = Variable(Tensor(np.random.normal(0, 1, (I.shape[0], self.ld))))
                    ohc = RelaxedOneHotCategorical(0.15,torch.ones([I.shape[0],self.y_len]))
                    y_prior = ohc.sample()
                ...
                if last_epoch is True:
                    mu, _ ,_,_,y_sample = self(depths_in,tnfs_in,z_prior, y_prior)[0:5]
                else:
