...
def get_cluster_score_bin_path(path_checkm_all,
                              path_bins,
                              bins_set):
    ...
    for sample in os.listdir(path_checkm_all):
        ...
        c_com_con=np.loadtxt(path_quality_s,delimiter='\t',skiprows=1,usecols=(0,1,2),dtype=str,ndmin=2)

        for row in c_com_con:
            cluster,com,con=row
            com,con=float(com),float(con)
            bin_name=cluster+'.fna'
            if bin_name in bins_set:
                cluster_score[cluster]=[com,con]
                bin_path[cluster+'.fna']=os.path.join(path_bins,sample,cluster+'.fna')
...
if __name__=='__main__':
    ...
    parser.add_argument("--cs_d", type=str, help="cluster_score dictionary will be stored here")
    parser.add_argument("--bp_d", type=str, help="bin_path dictionary will be stored here ")
