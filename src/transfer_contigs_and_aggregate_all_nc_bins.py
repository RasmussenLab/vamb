...
def main(path_run,
    ...
    nc_clusters_unchanged = mv_nc_not_r_nc_bins(cluster_not_r_contigs,
                       cluster_sample,
                       cluster_scores,
                       drep_folder,
                       bin_path)
    ...
    nc_clusters_unchanged_2, nc_clusters_ripped = choose_best_ripped_bin_and_mv_if_nc(cluster_r_scores,
                                       cluster_scores,
                                       cluster_r_pairs,
                                       cluster_sample,
                                       bin_path,
    ...
    # they are NC
    nc_clusters_ripped_single = mv_single_ripped_nc_bins(ripped_bins_scores_ar,
                                                        drep_folder,
                                                        cluster_r_pairs,
                                                        bin_path,
                                                        cluster_scores,
    ...
    write_nc_bin_scores(cluster_scores,
                        cluster_r_scores,
...
def get_cluster_sample(cluster_contigs,
                       bin_separator):
...
def mv_nc_not_r_nc_bins(cluster_not_r_contigs,
                     cluster_sample,
                     cluster_scores,
                     drep_folder,
                     bin_path):
    ...
    for cluster in cluster_not_r_contigs.keys():
        ...
        if (comp > 90 and cont < 5):
            ...
            trg_bin=os.path.join(path_run,drep_folder,cluster_sample[cluster],cluster+'.fna')
            print('Bin %s has no intersection with any other bin so it is directly moved from %s to %s'%(cluster,src_bin,trg_bin))
            shutil.move(src_bin,trg_bin) # this should be changed to mv once we assure it works properly
...
def mv_single_ripped_nc_bins(ripped_bins_scores_ar,
                            drep_folder,
                            cluster_r_pairs,
                            #path_bins_ripped,
                            bin_path,
    ...
    clusters_alredy_moved=set([element for pair in cluster_r_pairs for element in pair ] )
    for row in ripped_bins_scores_ar:
        ...
        if (comp > 90 and cont < 5):
            ...
            if os.path.isfile(src_bin):
                trg_bin=os.path.join(path_run,drep_folder,cluster_sample[cluster],cluster+'.fna')
                print('Bin %s was ripped because of meaningless edges or pairing and afterwards no intersection was shared with any other bin so it is moved from %s to %s'%(cluster,src_bin,trg_bin))
...
def choose_best_ripped_bin_and_mv_if_nc(cluster_r_scores,
                                       cluster_scores,
                                       cluster_r_pairs,
                                       cluster_sample,
                                       bin_path,
    ...
    for cluster_A_r,cluster_B_r in cluster_r_pairs:
        ...
        if (cluster_A_r in clusters_already_processed or cluster_B_r in clusters_already_processed):
            continue
        ...
        if keeper == 0:
            print('%s keeps the intersecting contigs whereas %s does not '%(cluster_A_r,cluster_B_r))                

            # bin_A keeps the contigs
            if (cluster_A_complet > 90 and cluster_A_cont < 5):
                bin_A_name = cluster_A_r+'.fna'
                src_bin=bin_path[bin_A_name]
                trg_bin=os.path.join(path_run,drep_folder,cluster_sample[cluster_A_r],bin_A_name)
                shutil.move(src_bin,trg_bin)
            ...
            if (cluster_B_r_complet > 90 and cluster_B_r_cont < 5):
                ...
                trg_bin=os.path.join(path_run,drep_folder,cluster_sample[cluster_B_r],bin_B_name)
                shutil.move(src_bin,trg_bin) 
                nc_clusters_ripped.add(cluster_B_r)
                print('%s looses the contigs so src_path is %s '%(bin_B_name,src_bin))     

        if keeper == 1:
            #print('%s keeps the intersecting contigs whereas %s does not '%(bin_B_name,bin_A_name))               
            print('%s keeps the intersecting contigs whereas %s does not '%(cluster_B_r,cluster_A_r))                
            ...
            if (cluster_A_r_complet > 90 and cluster_A_r_cont < 5):
                ...
                print('%s looses the contigs so src_path is %s '%(bin_A_name,src_bin))     
...
def write_nc_bin_scores(cluster_scores,
                        cluster_r_scores,
                        nc_clusters_unchanged,
                        nc_clusters_ripped, 
                        quality_file):
...
if __name__ == '__main__':
    ...
    path_run=opt.r#'/Users/mbv396/aamb/vamb_aamb_on_cami2/oral/manual_drep/vamb_aamb_cpu_jgi_oral_310122_run_2_manual_drep_disjoint2_copy/'


    path_clusters=opt.c#os.path.join(path_run,'aae_l_y_vamb_manual_drep_clusters.tsv')
    ...
    path_clusters_not_r=opt.cnr#os.path.join(path_run,'aae_l_y_vamb_manual_drep_not_ripped_clusters.tsv')

    with open(path_clusters_not_r) as file:
        cluster_not_r_contigs = vamb.vambtools.read_clusters(file)

    path_bins_ripped_checkm_quality_report=opt.sbr #os.path.join(path_bins_ripped,'checkm2_out','quality_report.tsv')
        

    ripped_bins_scores_ar= np.loadtxt(path_bins_ripped_checkm_quality_report,dtype=str,skiprows=1,ndmin=2)
