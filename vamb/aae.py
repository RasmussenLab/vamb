...
class Decoder(nn.Module):
    def __init__(self, h_n, ld, y_len, nsamples=0):
        ...
        if nsamples == 0:
            raise ValueError(
                "Number of samples should be provided to define the decoder output layer as well as the categorical latent dimension, not {}".format(
                    nsamples
                )
            )
        self.nsamples = nsamples
...
class Discriminator_y(nn.Module):
    def __init__(self, h_n, y_len, nsamples=0):
        ...
        if nsamples == None:
            raise ValueError(
                "Number of samples should be provided to define the decoder output layer, not {}".format(
                    nsamples
                )
            )
...
def train_model(
    ...
    for epoch_i in range(n_epochs):
        if epoch_i in batchsteps:
            batchsize *= 2
            dataloader_e = make_dataloader(
                depths, tnfs, batchsize, Shuffle=True, Cuda=_cuda
            )
        ...
        for i, (depths_in, tnfs_in) in enumerate(dataloader_e[0]):
            ...
            if _cuda:
                ohc = RelaxedOneHotCategorical(
                    ...
                    torch.ones([I.shape[0], y_len], device="cuda"),
                )
            else:
            ...
            vae_loss, ce, sse = calc_loss(
                depths_in, depths_out, tnfs_in, tnfs_out, num_samples
            )
            ...
            g_loss_adv_z = adversarial_loss(
                discr_z(z), valid
            )  # already done 3 lines above!!
            g_loss_adv_y = adversarial_loss(discr_y(y), valid)

            g_loss = (
                (1 - s_l) * vae_loss
                + (s_l * s_l_r) * g_loss_adv_z
                + (s_l * (1 - s_l_r)) * adversarial_loss(d_y_fake, valid)
            )  # possible bug, might affect the gradient computation!! use the g_loss_adv_y instead of adverarial_loss again!
        ...
        if (epoch_i + 1) == n_epochs:
            ...
            dataloader_val = make_dataloader(
                depths, tnfs, batch_size_val, Cuda=_cuda, Shuffle=False, Drop_last=False
            )
            path_clust = modelfile.replace("aae_model.pt", "aae_y_clusters.tsv")
            clust_y_dict, latent, mask = get_latents_y(
                encoder,
                contignames,
