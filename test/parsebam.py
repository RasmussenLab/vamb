...
try:
    x = vamb.parsebam._get_alternate_references(records[0]) == list()
except KeyError:
    pass
else:
    raise AssertionError('Should have failed when getting alternate references.')
...
ok_iterator = vamb.parsebam._filter_segments(file, minscore=None)
...
p, arr, length = vamb.parsebam._get_contig_rpkms(inpaths[0], outpath=None, minscore=50, minlength=100)
...
rpkm = vamb.parsebam.read_bamfiles(inpaths, minscore=50, minlength=100)
assert np.all(abs(rpkm - target_rpkm) < 1e-8)

rpkm = vamb.parsebam.read_bamfiles(inpaths, dumpdirectory='/tmp/dumpdirectory', minscore=50, minlength=100)
assert np.all(abs(rpkm - target_rpkm) < 1e-8)
