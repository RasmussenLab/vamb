...
- `Final_clusters.tsv`: Final set of clusters file product of running CheckM2 VAMB and AAMB bins per sample and subsequentely dereplicating the bins. Clusters contamination and completeness thresholds can be modified by setting the `max_cont` and `min_comp` in the `config.json` file.
-  `tmp` - Folder containing the intermediate files and directories generated:
   ...
   -  `snakemake_tmp` - Folder containing the all log files from the snakemake workflow. Those files are the evidences that each workflow rule has been executed, and some of them contain the actual stderr stdout of the commands executed by the rules. If encountering any problem with snakemake, check this folder for debugging.   
-  `log` - Folder containing all the log files from running the snakemake workflow in high computing cluster. If any snakemake rule fails to be executed, check the rule log for debugging.
