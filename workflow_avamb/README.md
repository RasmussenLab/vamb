...
- `abundance.npz`: File expressing contig abundances along samples calculated from the BAM files. Using this as input instead of BAM files will skip re-parsing the BAM files, which take a significant amount of time.
- `abundances` - Folder containing intermediate files generated during the `abundances.npz` file. ** 
- `avamb` - Folder containing the actual binning output files:
   ...
   - `avamb_manual_drep_disjoint_clusters.tsv`: Final set of near complete clusters file product of dereplicating VAMB and AAMB bins.
   - `bins`: folder containing all bins generated by the VAMB and AAMB per sample.
   ...
   - `contignames`: Text file containing a list of the contig names.
   ...
   - `NC_bins`: folder containing the final set of near complete bins by running Checkm2 on VAMB and AAMB bins per sample and subsequently dereplicating the bins. This folder also contains a `quality_report.tsv` file with the completeness and contamination of the final set of near complete bins.
...
- `contigs.flt.dict` - Dictionary file from the contigs fasta files. **
...
-  `contigs.flt.mmi` - A minimap indexes file. ** 
-  `log` - Folder containing all the log files from running the snakemake workflow on a cluster. If any snakemake rule fails to be executed, check the rule log for debugging.
-  `mapped` - Folder containing the sorted BAM files per sample, those BAM files were used by Avamb to generate the `abundance.npz` file, which expresses the contig abudnaces along samples. **
