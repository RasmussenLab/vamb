...
- `abundances` - Folder containing intermediate files generated during the `abundances.npz` file creation. ** 
- `avamb` - Folder containing the actual binning output files:
   ...
   - `avamb_manual_drep_disjoint_clusters.tsv`: Final set of near complete clusters file product of dereplicating VAMB and AAMB bins. Clusters contamination and completeness thresholds can be tuned by setting the `max_cont` and `min_comp` in the `config.json` file.
   ...
   - `NC_bins`: folder containing the final set of near complete bins by running Checkm2 on VAMB and AAMB bins per sample and subsequently dereplicating the bins. This folder also contains a `quality_report.tsv` file with the completeness and contamination of the final set of near complete bins. Bins contamination and completeness thresholds can be tuned by setting the `max_cont` and `min_comp` in the `config.json` file.
