...
def main(contig_names,
    ...
    cluster_contigs,graph_clusters,cluster_length,clusters_changed_but_not_intersecting_contigs=remove_meaningless_edges_from_pairs(graph_clusters,
                                                                                      cluster_length,
                                                                                      cluster_contigs,
                                                                                      contig_length)


    cluster_contigs, graph_clusters,cluster_length,clusters_changed_but_not_intersecting_contigs_2 = make_all_components_pair(graph_clusters,
                                                                              cluster_contigs,
                                                                              cluster_length,
                                                                              contig_length,
                                                                              cluster_sample)
    ...
    ripped_clusters_set = create_ripped_clusters_and_write_ripped_bins(graph_clusters,
                                                                       cluster_contigs, 
                                                                       cluster_sample,
                                                                       bin_path,
                                                                       path_ripped)    
    ...
    clusters_changed_but_not_intersecting_contigs_total={**clusters_changed_but_not_intersecting_contigs ,
    ...
    bin_path,cluster_contigs_not_ripped = find_remaining_clusters_ripped_and_write_ripped_bins(cluster_contigs_original,
    ...
    with open(path_clusters_not_ripped,'w') as file_:
        ...
        for cluster, contigs_nr in cluster_contigs_not_ripped.items():
            ...
            if contigs_o != contigs_nr:
                raise ValueError("Clusters that have been ripped remain in the non ripped set of clusters")
...
def get_graph_clusters(cluster_contigs, contig_length, cluster_length ):
    ...
    for i,cluster_i in enumerate(cluster_contigs.keys()):
        for j,cluster_j in enumerate(cluster_contigs.keys()):
            ...
            cluster_i_contigs, cluster_j_contigs=cluster_contigs[cluster_i],cluster_contigs[cluster_j]
            cluster_i_j_contigs_intersection=cluster_i_contigs.intersection(cluster_j_contigs)
            
            if len(cluster_i_j_contigs_intersection) > 0 :
                cluster_i_length,cluster_j_length=cluster_length[cluster_i],cluster_length[cluster_j]
                cluster_i_j_contigs_intersection_length=sum([contig_length[contig_k] for contig_k in cluster_i_j_contigs_intersection])
                intersection_length_ratio = cluster_i_j_contigs_intersection_length/min(cluster_i_length,cluster_j_length)
                graph_clusters.add_node(cluster_i)
                graph_clusters.add_node(cluster_j)
                graph_clusters.add_edge(cluster_i,cluster_j,weight=intersection_length_ratio)
                #print('%s and %s share '%(cluster_i,cluster_j))
                #print(cluster_i_j_contigs_intersection)                                               
    return graph_clusters

def remove_meaningless_edges_from_pairs(graph_clusters,
                                        cluster_length,
                                        cluster_contigs,
                                        contig_length,
                                        weight_thr=0.001
    ...
    for component in components:         
        if len(component) == 2 :
            ...
            if cl_i_cl_j_edge_weight > weight_thr:
                ...
                graph_clusters,cluster_contigs, cluster_length, cluster_updated = move_intersection_to_smaller_cluster(graph_clusters,
                                                                                    cl_i,
                                                                                    cl_j,
                                                                                    cluster_contigs,
                                                                                    cluster_length,
                ...
                clusters_changed_but_not_intersecting_contigs[cluster_updated]=cluster_contigs[cluster_updated]
    ...
    return cluster_contigs,graph_clusters, cluster_length, clusters_changed_but_not_intersecting_contigs

def move_intersection_to_smaller_cluster(graph_bins,
                                         cl_i,
                                         cl_j,
                                         cluster_contigs,
                                         cluster_length,
    ...
    if cl_i_length < cl_j_length:
        ...
        cluster_length[cl_j]= sum([contig_length[contig_i] for contig_i in cluster_contigs[cl_j]])
        cluster_updated=cl_j
    else:
        cluster_contigs[cl_i] = cl_i_contigs-cl_i_cl_j_intersection_contigs
        cluster_length[cl_i]= sum([contig_length[contig_i] for contig_i in cluster_contigs[cl_i]])
...
def make_all_components_pair(graph_clusters,
                             cluster_contigs,
                             cluster_length,
                             contig_length,
                             cluster_sample):
    ...
    for component in components:
        if len(component) > 2 :
            ...
            for cl_i in component:
                for cl_j in component-{cl_i}:
                    if graph_clusters.has_edge(cl_i,cl_j):
                        cl_i_cl_j_edge_weight = graph_clusters.get_edge_data(cl_i,cl_j)["weight"]
                        if  (cl_i_cl_j_edge_weight in weight_edges.keys() and weight_edges[cl_i_cl_j_edge_weight] != {cl_i,cl_j} ) :
            ...
            while component_len > 2:
                ...
                clusters_changed_but_not_intersecting_contigs[cluster_updated]=cluster_contigs[cluster_updated]
                component_len=max([len(nx.node_connected_component(graph_clusters,node_i)) for node_i in component])
...
def create_ripped_clusters_and_write_ripped_bins(graph_clusters,
                           cluster_contigs,
                           cluster_sample,
                           bin_path,
                           path_ripped
    ...
    for component in nx.connected_components(graph_clusters):
        ...
        print('Cluster %s and cluster %s share %i contigs '%(cl_i,cl_j,len(cluster_i_j_contigs_intersection)))
        ...
        write_ripped_bin(cl_i_contigs-cluster_i_j_contigs_intersection,
                         bin_i_path,
                         bin_i_ripped_path)
        
        print(bin_j_path,bin_j_ripped_path) 
        write_ripped_bin(cl_j_contigs-cluster_i_j_contigs_intersection,
                         bin_j_path,
                         bin_j_ripped_path)
...
def write_ripped_bin(contigs_cluster_ripped,
                     bin_i_path,
                     bin_i_ripped_path):
    """Write ripped bin"""
    contig_sequences=[]
...
def find_remaining_clusters_ripped_and_write_ripped_bins(cluster_contigs_original,
                                                        clusters_changed_but_not_intersecting_contigs_total,
                                                        cluster_sample,
                                                        bin_path,
                                                        path_ripped
    ...
    for cluster,contigs_r in clusters_changed_but_not_intersecting_contigs_total.items():
        contigs_o = cluster_contigs_original[cluster]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--co", type=str, help="path dereplicated clusters without ripped clusters")
    ...
    parser.add_argument("--log_nc_ripped_bins", type=str, help="path to og file that contains the # of ripped bins")

    opt = parser.parse_args()

    path_run=opt.r # '/Users/mbv396/aamb/vamb_aamb_on_almeida_hmp2/runs/vamb_aamb_on_almeida/'

    
    clusters_path= opt.ci  #  os.path.join(path_run,'aae_l_y_vamb_manual_drep_85_10_clusters.tsv')
    clusters_not_ripped_path= opt.co  #   os.path.join(path_run,'aae_l_y_vamb_manual_drep_85_10_not_ripped_clusters.tsv')
