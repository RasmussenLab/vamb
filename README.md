...
Additional number of high quality bins can be achieved when running Vamb together with Aamb as part of the Avamb snakemake workflow. Avamb is a metagenomics binning pipeline composed by aamb, an adversarial autoencoder for metagenomics binning, and the above described vamb, as well as a bin dereplication and quality filtering step that runs CheckM2. In the same way than vamb, avamb leverages sequence composition information from a contig catalogue and co-abundance information from BAM files by projecting those features into aamb and vamb latent spaces, where the metagenomics binning actually takes place. For more information, please check the [Avamb snakemake page] (https://github.com/RasmussenLab/avamb/tree/avamb_new/workflow_avamb).
